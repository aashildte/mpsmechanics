...
def _plot_circle(axis, pillar_coords, radius, time_step, start_indices):
    num_points = 200
...
def _plot_mesh_over_image(images, pillar_coords, radius, time, time_step):

    num_pillars = pillar_coords.shape[1]
    xplots = min(4, num_pillars)
    yplots = int(np.ceil(num_pillars / xplots))
    fig, axes = plt.subplots(yplots, xplots, figsize=(4 * xplots, 4 * yplots))
    ...
    for i in range(num_pillars):
        ...
        ci_subplots = _plot_circle(
            axis, pillar_coords[:, i], radius, time_step, start_indices
        )
...
def _plot_at_time_step(images, pillar_coords, radius, time, time_step, filename):
    displacement = pillar_coords

    if time_step is None:
        time_step = np.argmax(np.mean(np.linalg.norm(pillar_coords, axis=(2)), axis=1))

    _plot_mesh_over_image(images, pillar_coords, radius, time, time_step)

    plt.savefig(filename)
    plt.close("all")


def _make_animation(images, pillar_coords, radius, time, fname, animation_config):
    fig, update = _plot_mesh_over_image(images, pillar_coords, radius, time, 0)
    make_animation(fig, update, fname, **animation_config)


def _generate_param_filename(f_in, user_params):
    fname = generate_filename(
        f_in, f"pillars", user_params, "", subfolder="pillar_tracking",  # mp3 or png
    )
    return fname
...
def _read_input_data(f_in, param_list, overwrite_all):
    ...
    pillar_disp = read_prev_layer(f_in, track_pillars, param_list[:-1], overwrite_all)
    animation_config = get_animation_configuration(param_list[-1], mps_data)

    pillar_coords = pillar_disp["displacement_pixels"] + pillar_disp["initial_positions"]
    radius = pillar_disp["material_parameters"]["R"] / mps_data.info["um_per_pixel"]
...
def visualize_pillar_tracking(f_in, overwrite, overwrite_all, param_list):
    """
    ...
    animation_config, images, pillar_coords, radius, time = _read_input_data(
    ...
    if overwrite or not os.path.isfile(fname_png):
        _plot_at_time_step(images, pillar_coords, radius, time, time_step, fname_png)
        print("Plots of mesh over image at peak done; " + f"image saved to {fname_png}")
    else:
    ...
    if animate:
        if overwrite or not os.path.isfile(fname_mp4):
            ...
            _make_animation(
                images, pillar_coords, radius, time, fname_mp4, animation_config
            )

            print("Movie of mesh over image produced; " + f"movie saved to {fname_mp4}")
        else:

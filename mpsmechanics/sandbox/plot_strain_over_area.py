...
from mpsmechanics import read_prev_layer, analyze_mechanics, calc_norm_over_time


def plot_over_area(strain_data, im_data, time, M, N, label, title, output_pref):
    ...
    for m in range(1, M+1):
        ...
        for n in range(1, N+1):
            xfrom = (m-1)*xlen
            yfrom = (n-1)*ylen
            data_over_ns.append(1/area*calc_norm_over_time(strain_data[:, xfrom:xfrom + xlen, yfrom:yfrom + ylen]))
    ...
    for m in range(0, M+1):
        plt.plot([0, im_data.shape[1]-1], [m*xlen*blocksize, m*xlen*blocksize], 'w', linewidth=4)

    for n in range(0, N+1):
        plt.plot([n*ylen*blocksize, n*ylen*blocksize], [0, im_data.shape[0]-1], 'w', linewidth=4)
...
plot_over_area(strain_data, im_data, time, M, N, "Strain (-)", "Principal strain", \

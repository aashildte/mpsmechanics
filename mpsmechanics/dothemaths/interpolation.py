...
def interpolate_values_xy(x_coords, y_coords, org_data):
    ...
    assert (
        x_coords.shape[0] == org_data.shape[0]
    ), "Error: Dimension mismatch of x_coords and org_data."

    assert (
        y_coords.shape[0] == org_data.shape[1]
    ), "Error: Dimension mismatch of y_coords and org_data."

    fn_x = interpolate.interp2d(
        x_coords,
        y_coords,
        org_data[:, :, 0].transpose(),
        kind="linear",
    )
    fn_y = interpolate.interp2d(
        x_coords,
        y_coords,
        org_data[:, :, 1].transpose(),
        kind="linear",
    )

    fn_rel = lambda x, y: np.array(
        [float(fn_x(x, y)), float(fn_y(x, y))]
    )

...
def _data_to_dict(f_in, param_list, overwrite_all):
    ...
    for key in keys:
        ...
        metrics_data["Maximum average value"].append(
            data["metrics_max_avg"][key]
        )
        metrics_data["Average average value"].append(
            data["metrics_avg_avg"][key]
        )
        metrics_data["Maximum standard deviation"].append(
            data["metrics_max_std"][key]
        )
        metrics_data["Average standard deviation"].append(
            data["metrics_avg_std"][key]
        )

    return metrics_data


def _calculate_metrics_file(
    f_in, metrics_all, data_keys, param_list, overwrite_all
):

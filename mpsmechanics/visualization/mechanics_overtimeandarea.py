...
def setup_frame_gridspec(subplots_x, subplots_y, subdivision_x, subdivision_y):
    ...
    for i in range(1, subplots_x*subplots_y):
        inner = gridspec.GridSpecFromSubplotSpec(subdivision_x, subdivision_y, \
                    subplot_spec = outer[i], hspace=0.05, wspace=0.05)
...
def plot_image_subdivision(axis, image, values, subdivisions_xdir, subdivisions_ydir):
    axis.imshow(image, cmap='gray')
    
    xshape, yshape = values.shape[1:3]
    xlen, ylen = xshape // subdivisions_xdir, yshape // subdivisions_ydir
    blocksize = image.shape[0] // xshape

    for m in range(0, subdivisions_xdir+1):
        axis.plot([0, image.shape[1]-1], [m*xlen*blocksize, m*xlen*blocksize], 'w', linewidth=2)

    for n in range(0, subdivisions_ydir+1):
        axis.plot([n*ylen*blocksize, n*ylen*blocksize], [0, image.shape[0]-1], 'w', linewidth=2)


def plot_over_time(axes, values, time, subdivisions_xdir, subdivisions_ydir):

    assert len(values.shape) == 3, f"Error: Expected shape T x X x Y x 1; received {values.shape}"
    ...
    for _x in range(subdivisions_xdir):
        for _y in range(subdivisions_ydir):
            xfrom = _x*xlen
            yfrom = _y*ylen
            values_xy = 1/area*np.sum(values[:, xfrom:xfrom + xlen, \
                                                yfrom:yfrom + ylen], \
                                      axis=(1, 2))
            axes[_x][_y].plot(time, values_xy)


def plot_1d_values(spatial_data, time, subdivisions_xdir, subdivisions_ydir, label):
    """
    ...
    axes = setup_frame_gridspec(1, 2, subdivisions_xdir, subdivisions_ydir)

    plot_image_subdivision(axes[0], image, values, subdivisions_xdir, subdivisions_ydir)
    plot_over_time(axes[1], values, time, subdivisions_xdir, subdivisions_ydir)
...
def plot_2d_values(spatial_data, time, subdivisions_xdir, subdivisions_ydir, label):
    """
    ...
    magnitude, x_values, y_values, subtitles = \
            _get_2d_values(values)

    axes = setup_frame_gridspec(1, 4, subdivisions_xdir, subdivisions_ydir)

    plot_image_subdivision(axes[0], image, magnitude, subdivisions_xdir, subdivisions_ydir)
    plot_over_time(axes[1], magnitude, time, subdivisions_xdir, subdivisions_ydir)
    plot_over_time(axes[2], x_values, time, subdivisions_xdir, subdivisions_ydir)
    plot_over_time(axes[3], y_values, time, subdivisions_xdir, subdivisions_ydir)
...
def _get_2x2d_values(values):
    ...
    subtitles = ["component xx", "component xy", "largest singular value", "component yx", "component yy"]

    return xx_values, xy_values, sg_values, yx_values, yy_values, subtitles


def plot_2x2d_values(spatial_data, time, subdivisions_xdir, subdivisions_ydir, label):
    """
    ...
    axes = setup_frame_gridspec(2, 3, subdivisions_xdir, subdivisions_ydir)

    xx_values, xy_values, sg_values, yx_values, yy_values, subtitles = \
...
def _make_filename(f_in, metric, subdivisions_x, subdivisions_y, param_list):
...
def _plot_over_time(spatial_data, time, subdivisions_xdir, subdivisions_ydir, label, fname):

    values = spatial_data["derived_quantity"]

    plot_fn = get_plot_fun(values, \
            [plot_1d_values, plot_2d_values, plot_2x2d_values])
    plot_fn(spatial_data, time, subdivisions_xdir, subdivisions_ydir, label)

    plt.savefig(fname, dpi=300)


def visualize_over_time_and_area(f_in, overwrite, overwrite_all, param_list):
    """
    ...
    for metric in metrics:
        ...
        fname_png = _make_filename(f_in, metric, subdivisions_xdir, subdivisions_ydir, param_list)
        metadata, spatial_data, time = setup_for_key(mps_data, mc_data, metric)
        label = metadata["label"]

        if overwrite or (not os.path.isfile(fname_png)):
            _plot_over_time(spatial_data, time, subdivisions_xdir, subdivisions_ydir, \
                            label, fname_png)

...
def get_filename(input_file, quantity):
    folder = os.path.join(
        input_file[:-4], "mpsmechanics", "animation_over_time"
    )
...
def _setup_2plots(data, sigmas, key, x_lim, y_coord, time_step):
    ...
    for (i, sigma) in enumerate(sigmas):
        (linex,) = axes[0].plot(
            x_range,
            x_values[i][time_step],
            alpha=0.7,
            label=fr"$\sigma$ : {sigma}",
        )
        (liney,) = axes[1].plot(
            x_range, y_values[i][time_step], alpha=0.7
        )
...
def _setup_2x2plots(
    data, sigmas, key, x_lim, shift, y_coord, time_step
):
    ...
    vmax = 1.0 * max(
        [
            np.max(ux_values - shift),
            np.max(uy_values),
            np.max(vx_values),
            np.max(vy_values - shift),
        ]
    )
    ...
    for (i, sigma) in enumerate(sigmas):
        (lineux,) = axes[0].plot(
            x_range, ux_values[i][time_step], alpha=0.7
        )
        (lineuy,) = axes[1].plot(
            x_range,
            uy_values[i][time_step],
            alpha=0.7,
            label=fr"$\sigma$ : {sigma}",
        )
        (linevx,) = axes[2].plot(
            x_range, vx_values[i][time_step], alpha=0.7
        )
        (linevy,) = axes[3].plot(
            x_range, vy_values[i][time_step], alpha=0.7
        )

        for (_elem, _list) in zip(
            [lineux, lineuy, linevx, linevy],
            [ux_lines, uy_lines, vx_lines, vy_lines],
        ):
            _list.append(_elem)
...
def animate_over_time(bf_file, files, sigmas):
    ...
    y_coord = 30
    blocksize = (
        mps_data.frames.shape[0]
        // data[0]["all_values"]["displacement"].shape[1]
    )
    ...
    for key in ["displacement", "principal_strain"]:
        ...
        fig, update, num_time_steps = _setup_2plots(
            data, sigmas, key, x_lim, y_coord, 0
        )
        _make_animation(
            fig, update, framerate, num_time_steps, fname
        )

        peak = np.argmax(
            mc.calc_norm_over_time(data[3]["all_values"][key])
        )
    ...
    for (key, shift) in zip(
        ["deformation_tensor", "Green-Lagrange_strain_tensor"],
        shifts,
    ):
        fname = get_filename(bf_file, key)
        fig, update, num_time_steps = _setup_2x2plots(
            data, sigmas, key, x_lim, shift, y_coord, 0
        )
        _make_animation(
            fig, update, framerate, num_time_steps, fname
        )
        ...
        _setup_2x2plots(
            data, sigmas, key, x_lim, shift, y_coord, peak
        )
        plt.savefig(fname + ".png", bbox_inches="tight")
        plt.clf()
